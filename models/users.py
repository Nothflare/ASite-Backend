...
async def signup(username, password, email):
    ...
    res = await main.db(r"SELECT * FROM users WHERE username = '{}'".format(username))
    if len(res) > 0:
        return "Username already taken", 400
    else:
        res = await main.db(r"SELECT * FROM inactive_users WHERE username = '{}'".format(username))
        if len(res) > 0:
            return "Check email", 400
        password_hash = generate_password_hash(password)
    try:
        await main.db(r"INSERT INTO inactive_users (username, password, email) VALUES ('{}', '{}', '{}')".format(username, password_hash, email))
...
async def get_user(username, id):
    res = await main.db(r"SELECT user FROM users WHERE id = {}".format(id))
    return res
